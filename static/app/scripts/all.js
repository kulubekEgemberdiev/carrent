...
class CALENDAR {
    constructor(options) {
        ...
        this.elements = {
            ...
            carList: document.getElementsByClassName('calendar-car-list')
        };
...
// App methods
    ...
    drawDays() {
        ...
        days.forEach(day => {
            ...
            carsTemplate += `<li class="cars ${day.currentMonth ? '' : 'another-month'}${day.today ? ' active-day ' : ''}${day.selected ? 'selected-day' : ''}${day.hasEvent ? ' event-day' : ''}" data-day="${day.dayNumber}" data-month="${day.month}" data-year="${day.year}"></li>`
        });

        console.log(this.elements.cars);
    ...
    drawCars() {
        var carsTemlates = "";
        for(let i = 0; i < this.elements.carList; i++) {
            for (let i = 0; i < 30; i++) {
                carsTemplates = `<li class="cars ${day.currentMonth ? '' : 'another-month'}${day.today ? ' active-day ' : ''}${day.selected ? 'selected-day' : ''}${day.hasEvent ? ' event-day' : ''}" data-day="${day.dayNumber}" data-month="${day.month}" data-year="${day.year}"></li>`
